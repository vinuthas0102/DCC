...
const Metric: React.FC<{ label: string; value: React.ReactNode; valueCls?: string }> = ({
  ...
  <div className="min-w-0 shrink-0 border-r border-slate-100 pr-2 last:border-r-0 last:pr-0">
...
export const DemandListRecord: React.FC<DemandListRecordProps> = ({
  ...
  return (
    <motion.div
      ...
      className="group flex min-w-0 flex-col rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 hover:border-blue-300 hover:bg-blue-50/30 transition-colors"
    >
      {/* Row 1: identity + actions — single line, no wrapping */}
      <div className="flex min-w-0 items-center gap-1.5">
        ...
        <div className="min-w-0 flex-1 overflow-hidden">
          ...
          <div className="truncate text-[8px] leading-tight text-slate-400">{tile.object_ref}</div>
        </div>

        <div className="shrink-0 border-l border-slate-100 pl-2 w-[90px]">
        ...
        <div className="flex shrink-0 items-center gap-0.5 border-l border-slate-100 pl-2 w-[100px]">
          <span className="max-w-[44px] truncate rounded bg-blue-50 px-1 py-0.5 text-[8px] font-semibold text-blue-800">{tile.object_type}</span>
          <span className="max-w-[48px] truncate rounded bg-slate-100 px-1 py-0.5 text-[8px] font-semibold text-slate-600">{tile.demand_type_label}</span>
        </div>
      ...
      {/* Row 2: label-value details — single line, fixed widths */}
      <div className="flex min-w-0 items-center gap-2 border-t border-slate-100 pt-1 mt-1">

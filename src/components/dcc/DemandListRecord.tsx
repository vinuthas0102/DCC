...
const VALUE_CLS = 'text-[11px] font-bold text-slate-800 tabular-nums leading-snug';

const Metric: React.FC<{ label: string; value: React.ReactNode; valueCls?: string }> = ({
  label, value, valueCls = VALUE_CLS,
}) => (
  <div className="flex flex-col justify-center min-w-0 text-right leading-none">
...
export const DemandListRecord: React.FC<DemandListRecordProps> = ({
  ...
  return (
    <motion.div
      ...
      className="group grid grid-cols-12 items-center gap-2 px-3.5 py-2 w-full min-h-[52px] h-auto rounded-lg border border-slate-200 bg-white shadow-sm hover:shadow-md hover:border-blue-300 transition-all"
    >
      {/* Col 1-2: Status pill + Asset icon */}
      <div className="col-span-2 flex items-center gap-2 min-w-0">
        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold border ${st.bg} ${st.text} ${st.border} whitespace-nowrap leading-none`}>
      ...
      <div className="col-span-3 min-w-0 overflow-hidden border-r border-slate-100 pr-2 leading-snug">
        <div className="truncate text-[11px] font-bold text-slate-900 leading-snug">{tile.object_description || tile.object_ref}</div>
        <div className="truncate text-[10px] text-slate-400 leading-snug">{tile.object_ref}</div>
      </div>

      {/* Col 6-7: Client label & name */}
      <div className="col-span-2 min-w-0 overflow-hidden border-r border-slate-100 pr-2 leading-snug">
        <span className={LABEL_CLS}>CLIENT</span>
        <div className="mt-0.5 truncate text-[11px] font-semibold text-slate-700 leading-snug">{tile.owner_name}</div>
      </div>

      {/* Col 8: Category & type badges */}
      <div className="col-span-1 flex flex-col gap-1 min-w-0 border-r border-slate-100 pr-2">
        <span className="rounded bg-blue-50 px-1.5 py-0.5 text-[9px] font-semibold text-blue-800 whitespace-nowrap truncate text-center leading-none">{tile.object_type}</span>
        <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-600 whitespace-nowrap truncate text-center leading-none">{tile.demand_type_label}</span>
      </div>

      {/* Col 9: Dates */}
      <div className="col-span-1 flex flex-col justify-center gap-1 min-w-0 border-r border-slate-100 pr-2 leading-none">
        <Metric label="RUN" value={fmtDateShort(tile.demand_run_date)} valueCls="text-[10px] font-bold text-slate-600 tabular-nums leading-none" />
        <Metric label="DUE" value={fmtDateShort(tile.due_date)} valueCls={`text-[10px] font-bold tabular-nums leading-none ${tile.status === 'OVERDUE' ? 'text-red-600' : 'text-slate-600'}`} />
      </div>

      {/* Col 10-11: Financial breakdown */}
      <div className="col-span-2 flex items-center gap-2 min-w-0 border-r border-slate-100 pr-2 leading-none">
        <Metric label="BASE" value={fmtINR(tile.total_amount)} valueCls="text-[10px] font-bold text-slate-700 tabular-nums leading-none" />
        <Metric
          label="GST"
          value={tile.include_gst && tile.gst_amount > 0 ? fmtINR(tile.gst_amount) : '—'}
          valueCls={`text-[10px] font-bold tabular-nums leading-none ${tile.include_gst && tile.gst_amount > 0 ? 'text-slate-600' : 'text-slate-300'}`}
        />
        <Metric
          label="LATE"
          value={tile.overdue_amount > 0 ? fmtINR(tile.overdue_amount) : '—'}
          valueCls={`text-[10px] font-bold tabular-nums leading-none ${tile.overdue_amount > 0 ? 'text-red-600' : 'text-slate-300'}`}
        />
        <Metric
          label="PAYABLE"
          value={fmtINR(tile.amount_due)}
          valueCls={`text-[10px] font-bold tabular-nums leading-none ${tile.amount_due > 0 ? 'text-red-600' : 'text-emerald-600'}`}
        />
      </div>
